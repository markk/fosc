...
Examples from [http://abjad.mbrsi.org/api/tools/schemetools/Scheme.html](http://abjad.mbrsi.org/api/tools/schemetools/Scheme.html#abjad.tools.schemetools.Scheme)
...
Use this in certain override situations when LilyPond’s Scheme interpreter
treats unquoted strings as symbols instead of strings. The string must contain
no whitespace for this to work.
...
FoscScheme : FoscObject {
    ...
    *makeSpacingVector { |basicDistance=0, minimumDistance=0, padding=0, stretchability=0|
    ...
    Is true when formatting should format value absolutely verbatim. Whitespace,
    quotes, and all other parts of value are left intact.
    -------------------------------------------------------------------------------------------------------- */
    isVerbatim_ { |bool|
        isVerbatim = bool;
    }
